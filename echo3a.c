...
{
	struct pollfd pollfd[MAX_FD_COUNT];
	int i, ch;
	u_int burst = 1024, wait_link = 4;
	struct my_ring me[MAX_FD_COUNT];
	int n0, n1, ret;
	...
	while (!do_abort) {
        for (i = 0; i < MAX_FD_COUNT; i++) 
        {
    		pollfd[i].events = pollfd[i].revents = 0;
	    	pollfd[i].events |= POLLOUT;
		    pollfd[i].events |= POLLIN;
        }

		ret = poll(pollfd, MAX_FD_COUNT, 1000);
		if (ret < 0)
			continue;
        ...
        for (i = 0; i < MAX_FD_COUNT; i++)
        {
            if (pollfd[i].revents & POLLERR) {
			    D("error on fd0, rxcur %d@%d",
					me[i].rx->avail, me[i].rx->cur);
		    }
        }

        for (i = MAX_FD_COUNT - 1; i >= 0; i--)
        {
