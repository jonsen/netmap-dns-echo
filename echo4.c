...
{
	struct epoll_event event[MAX_EVENTS], *events;
	int nfd, efd,s;
	...
	for (i = 0; i < 2; i++)
	{ 
		event[i].data.fd = me[i].fd;
		event[i].events = EPOLLIN | EPOLLOUT ;
		s = epoll_ctl(efd, EPOLL_CTL_ADD, me[i].fd, &event[i]);
		if (s == -1) {
	...
	while (!do_abort) {
		...
		for (i = 0; i < nfd; i++)
		{
			if (events[i].data.fd == me[0].fd 
					&& ( events[i].events & EPOLLERR)) 
			{
				D("error on fd0, rxcur %d@%d",
						me[0].rx->avail, me[0].rx->cur);
			}
			if (events[i].data.fd == me[1].fd 
					&& ( events[i].events & EPOLLERR)) 
			{
				D("error on fd0, rxcur %d@%d",
			...
			if (events[i].data.fd == me[1].fd 
				...
				if (n1)
				{
					move(&me[1], &me[1], burst, 1);
					/* change DNS query content */
				}
				if (n0) 
				{
					move(&me[0], &me[1], burst, 0);
					/* swap packets */
				}
			}
			if (events[i].data.fd == me[0].fd 
					&& (events[i].events & EPOLLOUT)) {
				move(&me[1], &me[0], burst, 0);
				/* swap packets */
			}
		}
	}
